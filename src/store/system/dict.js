...
export const actions = {
  ...
  async deleteCatg({ commit, state }, { code }) {
    ...
    if (res.errcode === 0) commit(types.CATG_DELETED, { code });
    return res;
  },
  async updateCatg({ commit, state }, payload = {}) {
    ...
    if (res.errcode === 0) commit(types.CATG_UPDATED, res.data);
    return res;
  },
  ...
  async deleteItem({ commit, state }, { category, code }) {
    ...
    if (res.errcode === 0) commit(types.ITEM_DELETED, { code });
    return res;
  },
  async updateItem({ commit, state }, payload = {}) {
    ...
    if (res.errcode === 0) commit(types.ITEM_UPDATED, res.data);
    return res;
  },
